"""Validate the authored Kiro package, without network access or third-party packages."""
...
def main():
    course, sources, glossary = read("curriculum.json"), read("sources.json"), read("glossary.json")
    ...
    corpus = read("starter-labs/corpus.json")
    documents = {x["id"]: x for x in corpus}
    require(len(documents) == len(corpus), "Duplicate document IDs")
    dev, holdout = read("starter-labs/questions-dev.json"), read("starter-labs/questions-holdout.json")
    ...
    spec = importlib.util.spec_from_file_location("baseline", ROOT / "starter-labs/baseline.py")
    ...
    exact_case = [{"id": "unit", "question": "AUTH-401", "scope": "public", "goldDocIds": ["auth-runbook"]}]
    exact = module.evaluate(corpus, exact_case, 1)
    require(exact["meanRecallAtK"] == 1 and exact["meanReciprocalRank"] == 1, "Known-query metrics incorrect")
    ...
    print(json.dumps({"result": "passed", "lessons": len(lessons), "coreTasks": len(tasks),
                      "sources": len(sources), "glossaryTerms": len(glossary), "estimatedCoreHours": course["estimatedCoreHours"],
                      "syntheticDocuments": len(corpus), "developmentQuestions": len(dev), "holdoutQuestions": len(holdout),
                      "baselineChecks": "known/unknown retrieval, scope separation and metric denominators passed",
                      "developmentBaselineRecallAt3": metric["meanRecallAtK"],
                      "limits": "Structural/package and local baseline checks only. No application build, browser QA, cloud storage or deployment has been performed."}, indent=2))
